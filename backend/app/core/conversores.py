...
class ConversorDimensional:
    ...
    def converter_para_operacional(db: Session, produto_id: int, valor_base: float):
        ...
        Ex: converte Área (M2) para Comprimento (m).
        """
        ...
        sigla_resultado = "un"
        um_base = db.query(UnidadeMedida).filter(UnidadeMedida.id == produto.unidade_medida_id).first()
        if um_base:
            sigla_resultado = um_base.sigla

        if unidade_operacional:
            sigla_resultado = unidade_operacional.sigla
        ...
        try:
            if variavel == 'comprimento':
                ...
                return round(valor_base / largura_padrao, 4), sigla_resultado
                ...
                return round(valor_base / comp_padrao, 4), sigla_resultado
